...
vertex VertexOut vertexShader(constant VertexIn *vertexData[[ buffer(0) ]],
                              ...
                              constant float *viewportSizePointer [[ buffer(1) ]]) {
    VertexOut out;

    VertexIn in = vertexData[vertexID];
    
    vector_float2 viewportSize = vector_float2(viewportSizePointer[0], viewportSizePointer[1]);

    out.position = float4(in.vertexPosition, 0.0, 1.0);
    out.position.xy = out.position.xy / viewportSize;
